...
Consumer defaults live in `profile.models`. Explicit repo pins live in `profile.requestedModels`; task-role pins live in `profile.taskModels`. Resolve assignment as task pin, then the only automatic worker ladder `gpt-5.6-luna:max` → `profile.workerPolicy.elevatedModel` (`gpt-5.6-terra:max`). Copy an effective pin into both task `models.requested` and `models.assigned`. Sol Medium/High/Max are explicit worker pins only; historical Sol Medium packets remain valid. Plan/contract defects return to Refine without a worker-model escalation. Reviewer and orchestrator remain `gpt-5.6-sol:high`; validator uses `gpt-5.6-terra:medium`, explorer uses `gpt-5.6-luna:high`, and summarizer uses `gpt-5.6-luna:medium`.

...
def load_settings(repo: Path) -> dict[str, Any]:
    ...
    if merged.get("schemaVersion") != SCHEMA_VERSION:
        raise ValueError("schemaVersion 2 is unsupported by the schema-v4 runtime; replace the legacy bundle" if merged.get("schemaVersion") == 2 else f"unsupported telemetry settings schemaVersion: {merged.get('schemaVersion')!r}; expected 4")
...
def normalize_quality_observation(value: Mapping[str, Any], expected_task_id: str | None = None, task: Mapping[str, Any] | None = None) -> dict[str, Any]:
    if value.get("schemaVersion") != SCHEMA_VERSION:
        raise ValueError("schemaVersion 2 is unsupported by the schema-v4 runtime; replace the legacy bundle" if value.get("schemaVersion") == 2 else f"unsupported quality observation schemaVersion: {value.get('schemaVersion')!r}; expected 4")
